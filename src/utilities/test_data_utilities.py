...
class DataUtilities:
    ...
    async def create_transaction_portfolio(self, scope):
        ...
        portfolio = await self.transaction_portfolio_api.create_portfolio(
            scope, create_transaction_portfolio_request=request
        )
    ...
    def build_transaction_request(
        self, instrument_id, units, price, currency, trade_date, transaction_type
    ):
        return models.TransactionRequest(
            ...
            total_consideration=models.CurrencyAndAmount(
                amount=price * units, currency=currency
            ),
            source="Broker",
        )
    ...
    def build_adjust_holdings_request(
        self, instrument_id, units, price, currency, trade_date
    ):
        return models.AdjustHoldingRequest(
            instrument_identifiers={DataUtilities.lusid_luid_identifier: instrument_id},
            tax_lots=[
